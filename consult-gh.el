...
If repo, targetdir and name are not supplied interactively asks user for those values."
  ...
  (let ((repo (or repo (read-string "repo: " repo)))
        (targetdir (or targetdir (read-directory-name "target directory: " targetdir)))
        (name (or name (read-string "name: " name)))
        )
...
(run-hook-with-args 'consult-gh--repo-post-fork-hook forkrepo name)
))
