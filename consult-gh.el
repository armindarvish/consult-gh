...
This is used for all categories (repos, issues, prs, codes, files, etc.)"
...
(defvar consult-gh--preview-buffers-list (list)
  "List of currently open preview buffer.s")
...
(defvar consult-gh--repos-history nil
  "History variable for repos used in `consult-gh-issue-list',
and `consult-gh-pr-list'.")
...
(defface consult-gh-warning-face
  `((t :inherit 'warning))
  "the face to show currently open issues or PRS when listing or searching issues
 and PRS with `consult-gh';

By default inherits from `warning'.")

(defface consult-gh-error-face
  `((t :inherit 'error))
  "the face to show closed PRS when listing or searching PRS with `consult-gh';
...
"Converts Markdown style footnotes to org-mode style footnotes
...
"Converts Markdown style emphasis to org-mode style emphasis
...
"Makes a GitHub API call to get response in JSON format
...
"Converts a JSON object to a hash table
...
(defun consult-gh--get-current-orgs ()
  "Gets the organizations for currently logged in user
...
(defun consult-gh--get-gitignore-template-list ()
  "Lists name of .gitignore templates"
...
(defun consult-gh--get-license-template-list ()
  "Lists name of open source license templates.
...
(defun consult-gh--get-user-template-repos (&optional user)
  "Lists template repository for authenticated user.
...
(defun consult-gh--get-repo-from-directory (&optional dir)
  "Returns the full name of the GitHub repository in the current folder
...
(defun consult-gh--files-branches-hashtable-to-list (table repo)
  "Converts a hash table, TABLE with branches of REPO,
to a list of propertized text.

TABLE can for example be obtained by converting the json object from `consult-gh--files-get-branches' to a hash table
...
(defun consult-gh--files-branches-list-items (repo)
  "Returns REPO's information in propertized text format
...
(defun consult-gh--read-branch (repo)
  "Queries the user to select a branch of REPO
...
(defun consult-gh--files-get-trees (repo &optional branch)
  "Gets a recursive git \"tree\" of REPO and BRANCH in json object format
...
(defun consult-gh--files-table-to-list (table repo &optional branch)
  "Converts a hashtable containing git tree information of REPO and BRANCH
...
(defun consult-gh--files-list-items (repo &optional branch)
  "Fetches a list of files and directories in REPO and BRANCH.

Returns text with properties containing information about the file generated by `consult-gh--files-table-to-list'. This list can be passed to
...
(defun consult-gh--files-nodirectory-items (repo &optional branch)
  "Fetches a list of non-directory files in REPO and BRANCH from GitHub.

The format is propertized text that include information about the file generated by `consult-gh--files-table-to-list'.
...
for getting a list of file and directories."
  (let* ((branch (or branch "HEAD"))
    ...
    (mapcar (lambda (item) (unless (plist-get (cdr item) :size)
                             (setq items (delete item items))))
            items)
    items))

(defun consult-gh--files-get-content (url)
  "Fetches the contents of file at URL retrieved from GitHub api
...
(defun consult-gh--file-state ()
  "State function for file candidates in `consult-gh' (e.g. in
`consult-gh-find-file').
...
(defun consult-gh--file-annotate ()
  "Annotates each file candidate for `consult-gh-find-file'.

For more info on annotation refer to `consult''s manual, particularly
...
(defun consult-gh--file-group (cand transform)
  "Group function for file candidate.
...
(defun consult-gh--files-browse-url-action (cand)
  "Browses the url for a file candidate, CAND.
...
and the URL of the file on GitHub API.
It fetches the content from GitHub by `consult-gh--files-get-content'
and inserts it into a temporary file stored under `consult-gh-tempdir' in appropriate subdirectories for REPO.
...
To use this as the default action in `consult-gh-find-file', see `consult-gh--files-view-action'.
...
(defun consult-gh--files-view-action (cand)
  "Opens file candidate, CAND, in an Emacs buffer.

This is a wrapper function around `consult-gh--files-view'.

It parses CAND to extract relevant values
(e.g. repository, file path, url, ...) and passes them to `consult-gh--files-view'.
...
(defun consult-gh--files-save-file-action (cand)
  "Saves file candidate, CAND, to a file.

Its parses CAND to extract relevant information
(e.g. repository's name, file path, url, ...)
and passes them to `consult-gh--files-view',
then saves the buffer to file.

If `consult-gh-ask-for-path-before-save' is non-nil,
it queries the user for a file path,
otherwise it saves the file under
`consult-gh-default-save-directory'
with the buffer-file-name as the name of the file.

To use this as the default action on consult-gh's files,
set `consult-gh-file-action' to
...
(defun consult-gh--repo-group (cand transform)
  "Group function for repo candidates.
...
(defun consult-gh--repo-browse-url-action (cand)
  "Browses the url for a repo candidate, CAND.
...
the optional input, BUFFER. If BUFFER is nil, a buffer named by `consult-gh-preview-buffer-name' is used instead.

If `consult-gh-preview-buffer-mode' is set to either 'markdown-mode
or 'org-mode, it sets the major mode of the buffer accordingly,
otherwise it shows the raw text in fundamental-mode.
...
It runs the t“gh fork REPO --fork-name NAME”
using `consult-gh--command-to-string'."
...
(defun consult-gh--repo-create-scratch (&optional name owner description visibility make-readme gitignore-template license-key)
  "Creates a new repository on github from scratch
...
(defun consult-gh--repo-create-template (&optional name owner description visibility template)
  "Creates a new repository on github from TEMPLATE repo
...
(defun consult-gh--repo-create-push-existing (&optional directory name owner description visibility)
  "Creates a new repository on github from local repo in DIRECTORY
...
(defun consult-gh-repo-create-new (&optional name owner description visibility make-readme gitignore-template license-key template local-repo)
  "Creates a new repo, CAND, on GitHub.
...
(defun consult-gh--issue-list-format (string input highlight)
  "Formats minibuffer candidates for issues.
...
(defun consult-gh--issue-group-by-state (cand transform)
  "Group function for issue candidates.
...
(defun consult-gh--issue-browse-url-action (cand)
  "Browses the url for an issue candidate, CAND.
...
(defun consult-gh--issue-view-action (cand)
  "Opens the preview of an issue candidate, CAND.
...
(defun consult-gh--pr-list-format (string input highlight)
  "Formats minibuffer candidates for pull requests.
...
(defun consult-gh--pr-browse-url-action (cand)
  "Browses the url for a pull request candidate, CAND.
...
(defun consult-gh--search-code-format (string input highlight)
  "Formats minibuffer candidates for code.
...
(defun consult-gh--code-group (cand transform)
  "Group function for code candidates.
...
(defun consult-gh--code-browse-url-action (cand)
  "Browses the url for a code candidate, CAND.
...
(defun consult-gh--repo-list-transform (async builder)
  "Adds annotation to repo candidates in `consult-gh-repo-list'.
...
(defun consult-gh--search-repos-transform (async builder)
  "Adds annotation to repo candidates in `consult-gh-search-repos'.
...
"Builds gh command line for searching repositories with INPUT query
...
(defun consult-gh-search-repos (&optional initial noaction)
  "Interactively searches GitHub repositories.
...
of repositories for the entered username. The list of candidates in the completion table are dynamically updated as the user changes the input.
...
This simply passes `consult-gh-default-orgs-list' to `consult-gh-orgs',
...
If REPPOS or TARGETDIR are not supplied, interactively asks user
to pick them."
  (interactive)
  (let* ((consult-gh-prioritize-local-folder (if (eq consult-gh-prioritize-local-folder 'suggest) consult-gh-prioritize-local-folder nil))
         (repos (or repos (substring-no-properties (car (consult-gh-search-repos nil t)))))
...
(defun consult-gh--issue-list-transform (async builder)
  "Adds annotation to issue candidates in `consult-gh-issue-list'.
...
(defun consult-gh--issue-list-builder (input)
  "Builds gh command line for listing issues the INPUT repository.
...
in an async process and returns the results (list of issues for a repository)
as a completion table in minibuffer. The completion table gets dynamically
updated as the user types in the minibuffer to change the entry.
Each candidate in the minibuffer is formatted by `consult-gh--issue-list-transform' to add annotation and other info
to the candidate.
...
(defun consult-gh-issue-list (&optional initial noaction)
  "Interactively lists issues of a GitHub repository.
...
minibuffer (expected format is “OWNER/REPO”), then fetches the list of issues
of that repository and present them as a minibuffer completion table for
selection. The list of candidates in the completion table are dynamically
updated as the user changes the minibuffer input.
...
by typing `--` followed by command line arguments. For example the user can enter the following in the minibuffer:
...
(defun consult-gh--search-issues-transform (async builder)
  "Adds annotation to issue candidates in `consult-gh-search-issues'.
...
(defun consult-gh--search-issues-builder (input)
  "Builds gh command line for searching issues of INPUT query."
...
(defun consult-gh--async-search-issues (prompt builder &optional initial)
  "Seacrhes GitHub issues asynchronously
...
(defun consult-gh-search-issues (&optional initial repo noaction)
  "Interactively searches GitHub issues.

This is an interactive wrapper function around
`consult-gh--async-search-issues'. With prefix ARG, first search for a repo
using `consult-gh-search-repos', then search issues of only
that selected repo.

It queries the user for a search term in the minibuffer, then fetches the
list of possible GitHub issue for the entered query and presents them as a minibuffer completion table for selection. The list of candidates in the completion table are dynamically updated as the user changes the entry.
...
(defun consult-gh--pr-list-transform (async builder)
  "Adds annotation to issue candidates in `consult-gh-pr-list'.
...
(defun consult-gh--pr-list-builder (input)
  "Builds gh command line for listing pull requests of the INPUT repository.
...
(defun consult-gh-pr-list (&optional initial noaction)
  "Interactively lists pull requests of a GitHub repository.
...
(defun consult-gh--search-prs-transform (async builder)
  "Adds annotation to pr candidates in `consult-gh-search-prs'.
...
(defun consult-gh--search-prs-builder (input)
  "Builds gh command line for searching pull requests of INPUT query"
...
(defun consult-gh--async-search-prs (prompt builder &optional initial)
  "Seacrhes GitHub pull requests asynchronously.
...
the input) as a completion table in minibuffer. The completion table gets dynamically updated as the user types in the minibuffer.
Each candidate in the minibuffer is formatted by
...
(defun consult-gh-search-prs (&optional initial repo noaction)
  "Interactively searches GitHub pull requests.
...
(defun consult-gh--search-code-transform (async builder)
  "Adds annotation to code candidates in `consult-gh-search-code'.
...
(defun consult-gh--search-code-builder (input)
  "Builds gh command line for searching code with INPUT query."
...
(defun consult-gh--async-search-code (prompt builder &optional initial)
  "Seacrhes GitHub codes asynchronously
...
(defun consult-gh-search-code (&optional initial repo noaction)
  "Interactively searches GitHub codes.
