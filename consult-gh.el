...
(let* (
  ...
  (if (eq (car response) 0)
     (completion-table-in-turn (delete-dups (sort (consult-gh--files-table-to-list (consult-gh--api-json-to-table (cadr response) "tree") repo branch) 'string<)) my:make-table)
    (message (cadr response)))))
