...
;; (defun consult-gh--files-table-to-list (table repo &optional branch)
;;    (let ((branch (or branch "HEAD")))
;;     (mapcar (lambda (el) (propertize (concat repo "/" (gethash :path el)) ':repo repo ':branch branch ':url (gethash "url" el) ':path (gethash :path el) ':size (gethash :size el))) table)))
...
(defun consult-gh--files-sibling-pred (candidates current string)
...
(defun consult-gh--files-completing-read (current table &rest args)
  (let ((comp (all-completions current table)))
    (print (format "%s:\n%s\n from %s table" current comp table))
    (if (> (length comp) 1)
        (progn
        (unless (equal current "") (setq table (append (list (string-join (butlast (butlast (string-split current "/"))) "/")) (all-completions current table))))
        (consult--read table
                      :prompt "Select a File: "
                      :require-match t
                      :default current
...
(let* (
       ...
       (items (mapcar (lambda (item) (if (not (get-text-property 0 :size item)) (concat item "/") item)) (remove "" items)))
...
;;(all-completions "branding/logo" table)
;;items
;;table
)
