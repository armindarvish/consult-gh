...
#+begin_src emacs-lisp
(consult-gh--code-search-regexp-compiler (car (consult--command-split "import requests")))
#+end_src
...
#+begin_src emacs-lisp
(consult-gh--api-get-json "repos/armindarvish/consult-gh/contents/consult-gh-forge.el")
#+end_src


#+begin_src emacs-lisp
(apply #'consult-gh--command-to-string '("issue" "list"))
#+end_src

#+begin_src emacs-lisp
(apply #'append '("--sort" "created"))
#+end_src



#+begin_src emacs-lisp
(funcall consult--regexp-compiler (car (consult--command-split "consult-gh new -- topic:github")) 'emacs t)
#+end_src

#+RESULTS:
:results:
(("consult-gh" "new") . #[128 "\302\301\303\300\"\"\207" [(("consult-gh" "new") t) consult--highlight-regexps apply append] 6 "

(fn &rest ARGS2)"])
:end:

#+begin_src emacs-lisp
(consult--join-regexps (car
(funcall consult--regexp-compiler (car (consult--command-split "consult-gh -- topic:github")) 'emacs t)) 'emacs)
...
#+begin_src emacs-lisp
(consult--split-escaped "consult-gh -- topic:github")
#+end_src

#+RESULTS:
:results:
("consult-gh" "--" "topic:github")
:end:

#+begin_src emacs-lisp
(consult-gh--files-get-content "repos/armindarvish/consult-gh/contents/consult-gh-forge.el")
#+end_src


#+begin_src emacs-lisp
(let* ((json (cadr (consult-gh--call-process "api" "-H" "Accept: application/vnd.github.text-match+json" "search/code?q=consult-gh" "--jq" "{repository: .items.[].repository, text_matches: .items.[].text_matches }")))
       (table (consult-gh--api-json-to-hashtable json :items)))
(with-current-buffer (get-buffer-create "*my:test-buffer*")
  (erase-buffer)
  (goto-char (point-min))
...
#+begin_src emacs-lisp
(with-current-buffer (get-buffer-create "*my:test-buffer1*")
  (erase-buffer)
  (goto-char (point-min))
  (insert
(consult-gh--highlight-match "consult-gh" (base64-decode-string (consult-gh--api-json-to-hashtable (cadr (consult-gh--call-process "api" "-H" "Accept: application/vnd.github+json" "https://api.github.com/repositories/4542716/contents/pkgs/applications/editors/emacs/elisp-packages/manual-packages/consult-gh/default.nix?ref=58aa6737113478177ad861c2ff20e10bc8ce12af")) :content)) nil))
  )
#+end_src
...
#+begin_src emacs-lisp
(consult--async-split-initial "armindarvish")
#+end_src

#+begin_src emacs-lisp
(car (funcall consult--regexp-compiler "consult-gh" 'emacs 'basic))
#+end_src
...
#+begin_src emacs-lisp
(car (consult--default-regexp-compiler "consult-gh" 'basic t))
#+end_src
...
#+begin_src emacs-lisp
(consult--default-regexp-compiler "\\(consult-gh.*\\|consult-web-.*?\\) vertico" 'emacs t)
#+end_src

#+RESULTS:
:results:
(("\\(consult-gh.*\\|consult-web-.*?\\)" "vertico") . #[128 "\302\301\303\300\"\"\207" [(("\\(consult-gh.*\\|consult-web-.*?\\)" "vertico") t) consult--highlight-regexps apply append] 6 "

(fn &rest ARGS2)"])
:end:

#+begin_src emacs-lisp
(string-split "\\(consult-gh.*\\|consult-web-.*?\\) vertico -- topic:github" "[\t\n\r\f\s\v]+")
#+end_src

#+RESULTS:
:results:
("\\(consult-gh.*\\|consult-web-.*?\\)" "vertico" "--" "topic:github")
:end:

#+begin_src emacs-lisp
  (consult--split-escaped (car (consult--command-split "magit status -- topic:github")))
#+end_src
