...
Currently, installing with =package-vc-install= or using =:vc= in =using-package= macro does not work. This is beacuse of how =package-vc-install= handles dependencies. It tries to install all the =Package-Requires= declarations in every =*.el= file in the root directory. _This is not compatible with a multi-package repo like consult-gh_ because the other .el files (like consult-gh-embark) have a dependency on consult-gh and that creates some looped self-dependencies in pakcage-vc-install. If you try to install consult-gh with package-vc-install either you get some errors or get the version from MELPA installed as a dependency.
