...
- =consult-gh-embark=: To add embark actions for consult-gh:
...
- =consult-gh-forge=: to integrate =consult-gh= with [[https://github.com/magit/forge][magit/forge]]:
...
*** Customization
For the full list and descriptios of the custom variables please refer to the documentation or use emacs's customize interface by =M-x customize= or =(customize 'consult-gh)=. Here is a list of the most important custom varibales with brief description:
...
**** =consult-gh-search-repos-args=
A list of command line arguments passed to "gh" command line program to search repositories. By default it is set to ='("search" "repos")= so it searches all repositories. Customizing this varable is useful if you want to add some additional conditions (i.e. command line arguments) permanently fto your repository searches. For example if you want to limit repository searches to a specific user or organization, etc.

**** =consult-gh-repo-list-args=
Similar to  =consult-gh-search-repos-args=, but for listing repos. The default is ='("repo" "list")=

**** =consult-gh-search-issues-args=
Similar to  =consult-gh-search-repos-args=, but for searching issues. The default is ='("search" "issues")=
**** =consult-gh-issue-list-args=
Similar to  =consult-gh-search-issues-args=, but for listing issues. The default is ='("issue" "list")=


**** =consult-gh-search-prs-args=
Similar to  =consult-gh-search-prs-args=, but for searching pull requests. The default is ='("search" "prs")=.

**** =consult-gh-pr-list-args=
Similar to  =consult-gh-search-prs-args=, but for listing pull requests. The default is ='("pr" "list")=


**** =consult-gh-search-code-args=
Similar to  =consult-gh-search-repos-args=, but for searching code. The default is ='("search" "code")=


**** =consult-gh-search-issues-args=
Similar to  =consult-gh-search-repos-args=, but for searching issues.

**** =consult-gh-notifications-args=
Similar to  =consult-gh-search-repos-args=, but for getting notifications from github cli. By default this uses a =gh api= call with =--tmeplate= formating switch to get all the relevant information about notifications.

**** =consult-gh-notifications-show-unread-only=
A boolean determining whether to hide read notifications or not (show all notifications).

**** =consult-gh-dashboard-items-functions=
A list of functions that are used to collect items that should be shown in the dashboard. Customizing this variable is useful if you want to change the items you normally see in the dashboard.
...
This variable defines whether =gh= uses the git repo in the local folder to select/guess the repository for commands like =consult-gh-find-file= and =consult-gh-issue-list=, etc. Note that everything is still retrieved from the remote (for example =consult-gh-find-file= would fetch the contents of the repo from remote and not from the local folder) but the local repo name is used instead of querying the user.
You can set this variable to ='suggest=, =t=, or =nil=
...
This is a list of default organizations you want to access frequently. I set this variable to organizations I am part of (a.k.a. to look at my own repositories). Conviniently the function =consult-gh--get-current-orgs= is provided to get a list of organizations of the currently looged in and selected user. But you can also add any other account you want to look at frequently to =consult-gh-default-orgs-list= list. Here is an example of setting the default list to "alphapapa" and "systemcrafters":
...
**** =consult-gh-repo-preview-mode=
This is the major-mode used to show repository previews. By default, it is set to nil, which uses the major-mode associated with the extension of the README file. But you can set it to ='markdown-mode= or ='org-mode= to force showing previews in the mode you like.

**** =consult-gh-issue-preview-mode=
This is the major-mode used to show issue/pr previews. By default, it is set to ='markdown-mode=, but if you prefer to see the contents in =org-mode= format, you can set this variable to ='org-mode=. Be aware that currently, the org-mode conversion is done with some simple regex replacements and while the performance is decent and the converted version is perfectly understandable, the org conversion may cause some inaccuracies. If you want to try this you can do:

#+begin_src emacs-lisp
(setq consult-gh-issue-preview-mode 'org-mode)
#+end_src
...
**** =consult-gh-discussion-action=
Similar to =consult-gh-issue-action= and =consult-gh-pr-action= but for discussion topics.

**** =consult-gh-notifications-action=
Similar to =consult-gh-repo-action= but for notifications. By default it is bound to =consult-gh--notifications-action= which passes notifications to defult actions of issues or prs according to the item type.

**** =consult-gh-dashboard-action=
Similar to =consult-gh-notifications-action= but for items in the dashboard. By default it is bound to =consult-gh--dashboard-action= which passes items to defult actions of issues or prs according to the item type.

**** =consult-gh-notifications-group-function=
The function used for grouping notifications. By default it is set to use =consult-gh--notifications-group-by-date= which groups the notificaitons by their date. But it can be customized to use item types (e.g. issues or pullrequests), or reason for notification (e.g. a mention vs. review-request), etc.

**** =consult-gh-dashboard-group-function=
Similar to =consult-gh-notifications-group-function= but for dashboard items. By default it is set to use =consult-gh--dashboard-group-by-reason= which groups the items in the dashboard by the reason of their relevance (e.g. mentions, review-request, assignment, etc.)
