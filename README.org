...
=consult-gh= offers the following "MAIN" interactive commands (This is not a comprehensive list, just hte most commonly used commands).
...
(use-package consult-gh
  ...
  (consult-gh-dashboard-action #'consult-gh--dashboard-action) ;;open dashboard items using default actions for issue/pr
  (consult-gh-default-interactive-command #'consult-gh-transient)
  :config
...
| key   | command                                                 |
|-------+---------------------------------------------------------|
| RET   | open the file on currnet line                           |
| m     | mark files on current line                              |
| u     | unmark file on current line                             |
| U     | unmark all marked files                                 |
| t     | toggle marks (make marked files unmarked and vise versa |
| D     | delete file                                             |
| C     | copy file                                               |
| R     | rename file                                             |
| ^     | up directory                                            |
...
*NOTE*: Currently, this dooes not work if you install consult-gh from MELPA. I am going to submit consult-gh-nerd-icons as a separate package to MELPA, but that is not done yet. For now this is only available if you use a package manager like Straight.el and install directly from consult-gh's repo.
