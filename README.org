...
This variable is boolean determining whether Consult-GH shows previews or not. By default this is set to nil because loading repository views or file contents can be resource-heavy depending on the size of the content. Therefore it is left to the user to decide whether showing previews is suitable for their use-case and their setup or not. If you don't want to turn previews on globally but still want to be able to see a preview on demand, then look at below.
...
Example Usage: run =M-x consult-gh-orgs= and look for "armindarvish", "minad", "systemcrafter", ...) with the ability to look-up multiple accounts at the same time using crm separator (similar to completing-read-multiple).


* Template From Consult
